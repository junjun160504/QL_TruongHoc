...
public:
  ...
  void nhapHS() {
    ...
    cout << "Nhap so dien thoai: ";
    cin >> sdt;
    cout << "Nhap so dien thoai phu huynh: ";
    cin >> sdtPH;
  }
  void xuatHS() const {
    cout << left << setfill(' ') << setw(10) << maHS << setw(20) << tenHS
         << setw(10) << lop << setw(20) << diaChi << setw(15) << sdt << setw(15)
         << sdtPH << endl;
  }
};
...
public:
  ...
  float diemTB() const {
    return round((DM + DCC + DKT * 2 + DT * 3) / 7 * 100) /
           100; // làm tròn đến 2 số thập phân
  }
  void xuatDiem() const {
    cout << left << setfill(' ') << setw(10) << maHS << setw(10) << maMH
         << setw(8) << DM << setw(8) << DCC << setw(8) << DKT << setw(8) << DT
         << setw(8) << diemTB() << endl;
  }
};
...
vector<GiaoVien> docGiaoVienTuFile(const string &tenFile) {
  ...
  string line;
  while (
      getline(filein, line)) { // đọc từng dòng trong file và lưu vào biến line
    stringstream ss(line); // tạo ra một string stream từ dòng vừa đọc, Chuyển
                           // dòng đọc được thành luồng chuỗi
    GiaoVien gv;
    getline(ss, gv.maGV, ',');
    getline(ss, gv.tenGV, ',');
...
void ghiFileGV(const string &tenFile, const vector<GiaoVien> &dsgv) {
  ...
  for (const auto &gv : dsgv) { // duyệt qua từng phần tử trong vector
    fileout << gv.maGV << "," << gv.tenGV << "," << gv.monDay << "," << gv.sdt
            << endl;
  }
  fileout.close();
}
...
void ghiFileHS(const string &tenFile, const vector<HocSinh> &dshs) {
  ...
  for (const auto &hs : dshs) { // duyệt qua từng phần tử trong vector
    fileout << hs.maHS << "," << hs.tenHS << "," << hs.lop << "," << hs.diaChi
            << "," << hs.sdt << "," << hs.sdtPH << endl;
  }
  fileout.close();
}
...
void ghiFileBD(const string &tenFile, const vector<BangDiem> &dsbd) {
  ...
  for (const auto &bd : dsbd) { // duyệt qua từng phần tử trong vector
    fileout << bd.maHS << "," << bd.maMH << "," << bd.DM << "," << bd.DCC << ","
            << bd.DKT << "," << bd.DT << endl;
  }
  fileout.close();
}
  ...
  do {
    ...
    case 3: // nhập bảng điểm
    {
      ...
      for (int i = 0; i < dshs.size(); i++) {
        BangDiem bd;
        cout << "Nhap diem cho hoc sinh " << dshs[i].maHS << " - "
             << dshs[i].tenHS << ": " << endl;
        bd.maHS = dshs[i].maHS;
        bd.maMH = monHoc;
        cout << "Nhap diem mieng: ";
    ...
    case 4: {
      ...
      cout << "\n=========== DANH SACH GIAO VIEN ===========\n";
      cout << left << setw(15) << "Ma giao vien" << setw(25) << "Ten giao vien"
           << setw(20) << "Mon day" << endl;
      for (const auto &gv : dsgv) {
        cout << left << setw(15) << gv.maGV << setw(25) << gv.tenGV << setw(20)
             << gv.monDay << endl;
      }
      ...
      if (tolower(save) == 'y') {
        ...
        fileout << "\n=========== DANH SACH GIAO VIEN ===========\n";
        fileout << left << setw(15) << "Ma giao vien" << setw(25)
                << "Ten giao vien" << setw(20) << "Mon day" << endl;
        for (const auto &gv : dsgv) {
          fileout << left << setw(15) << gv.maGV << setw(25) << gv.tenGV
                  << setw(20) << gv.monDay << endl;
        }
        fileout.close();
    ...
    case 5: {
      ...
      cout << "\n=================== DANH SACH HOC SINH ===================\n";
      cout << left << setw(15) << "Ma hoc sinh" << setw(25) << "Ten hoc sinh"
           << setw(10) << "Lop" << setw(20) << "Dia chi" << setw(15) << "SDT"
           << setw(20) << "SDT phu huynh" << endl;
      for (const auto &hs : dshs) {
        cout << left << setw(15) << hs.maHS << setw(25) << hs.tenHS << setw(10)
             << hs.lop << setw(20) << hs.diaChi << setw(15) << hs.sdt
             << setw(20) << hs.sdtPH << endl;
      }
      ...
      if (tolower(save) == 'y') {
        ...
        fileout
            << "\n=================== DANH SACH HOC SINH ===================\n";
        fileout << left << setw(15) << "Ma hoc sinh" << setw(25)
                << "Ten hoc sinh" << setw(10) << "Lop" << setw(20) << "Dia chi"
                << setw(15) << "SDT" << setw(20) << "SDT phu huynh" << endl;
        for (const auto &hs : dshs) {
          fileout << left << setw(15) << hs.maHS << setw(25) << hs.tenHS
                  << setw(10) << hs.lop << setw(20) << hs.diaChi << setw(15)
                  << hs.sdt << setw(20) << hs.sdtPH << endl;
        }
        fileout.close();
    ...
    case 8: // báo cáo kết quả học tập theo lớp
    {
      ...
      if (in == 'Y' || in == 'y') {
        ...
        fileout << "\n================================= BAO CAO KET QUA HOC "
                   "TAP - LOP "
                << lop << " =================================\n";
        fileout << left << setw(15) << "Ma hoc sinh" << setw(20)
                << "Ten hoc sinh";
        for (const auto &mon : DSMonHoc) {
          fileout << setw(10) << mon;
        }
    ...
    case 9: // In danh sách điểm trung bình của học sinh
    {
      ...
      for (const auto &hs : hsTheoLop) {
        ...
        float diemTB =
            (soMon > 0) ? (tongDiem / soMon) : -1; // Tính điểm trung bình
      ...
      if (in == 'Y' || in == 'y') {
        ...
        fileDTB << "\n================================= BAO CAO DIEM TRUNG "
                   "BINH HOC SINH - LOP "
                << lop << " =================================\n";
        fileDTB << left << setw(15) << "Ma hoc sinh" << setw(20)
                << "Ten hoc sinh" << setw(15) << "Diem TB" << endl;
    ...
    case 10: { // Hiển thị học sinh có điểm trung bình cao nhất
      ...
      cout << left << setw(10) << "Ma HS" << setw(20) << "Ten HS" << setw(10)
           << "Lop" << setw(8) << "TB" << endl;

      for (const auto &hs : hsMaxDiemTB) {
        cout << left << setw(10) << hs.maHS << setw(20) << hs.tenHS << setw(10)
             << hs.lop;
    ...
    case 12: // xếp loại học lực
    {
      ...
      cout << "\n============ XEP LOAI HOC LUC - LOP " << lop
           << " ============\n";
      cout << left << setw(15) << "Ma hoc sinh" << setw(20) << "Ten hoc sinh"
           << setw(10) << "Diem TB" << setw(15) << "Hoc luc" << endl;
      ...
      for (const auto &hs : hsTheoLop) {
        ...
        // Hiển thị kết quả
        cout << left << setw(15) << hs.maHS << setw(20) << hs.tenHS << setw(10)
             << round(diemTB * 100) / 100 << setw(15) << hocLuc << endl;
      }
      cout << "========================================================\n";
      ...
      if (save == 'Y' || save == 'y') {
        ...
        fileout << "\n============ XEP LOAI HOC LUC - LOP " << lop
                << " ============\n";
        fileout << left << setw(15) << "Ma hoc sinh" << setw(20)
                << "Ten hoc sinh" << setw(10) << "Diem TB" << setw(15)
                << "Hoc luc" << endl;
        ...
        for (const auto &hs : hsTheoLop) {
          ...
          fileout << left << setw(15) << hs.maHS << setw(20) << hs.tenHS
                  << setw(10) << round(diemTB * 100) / 100 << setw(15) << hocLuc
                  << endl;
        }
        fileout << "========================================================\n";
        fileout.close();
